...
distribn_t* LBM<LatticeType>::WallShearStressMagn_Inner_Type6_GPU = nullptr;
		...
		template<class LatticeType>
			bool LBM<LatticeType>::Read_DistrFunctions_CPU_to_GPU_totalSharedFs()
			{
				...
				// No neighbouring GPU ranks (e.g. single-GPU run): nothing to copy.
				if (totSharedFs == 0 || mLatDat->neighbouringProcs.empty()) return true;
...
template<class LatticeType>
					...
					iolets::BoundaryValues* iOutletValues, const util::UnitConverter* iUnits)
			{
				...
				if((uint64_t)mLatDat->GetLocalFluidSiteCount() * LatticeType::NUMVECTORS > 2147483647ULL){ fprintf(stderr, "ERROR: geometry too large for int32 neighbour indices\n"); initialise_GPU_res = false; return initialise_GPU_res; }
				int32_t* Data_int64_Neigh_d = new int32_t[(size_t)mLatDat->GetLocalFluidSiteCount() * LatticeType::NUMVECTORS];	// site_t (type int64_t)
				...
				for (unsigned int l = 0; l < LatticeType::NUMVECTORS; l++)
				{
					for (site_t i = 0; i < mLatDat->GetLocalFluidSiteCount(); i++)
					{
						...
						if (neigh_Index_Heme < mLatDat->GetLocalFluidSiteCount() * LatticeType::NUMVECTORS )
						{
								...
								Data_int64_Neigh_d[(int64_t)l * mLatDat->GetLocalFluidSiteCount() + i] = (int32_t)neigh_Address_Index;
						}
						else{
								Data_int64_Neigh_d[(int64_t)l * mLatDat->GetLocalFluidSiteCount() + i] = (int32_t)neigh_Index_Heme;
						}
				...
				status = deviceMalloc((void**)&GPUDataAddr_int64_Neigh_d, nArray_Neigh * sizeof(int32_t));
				if(!status){
				...
				status = deviceMemcpy(GPUDataAddr_int64_Neigh_d, Data_int64_Neigh_d, nArray_Neigh * sizeof(int32_t), memcpyHostToDevice);
				if(!status){
      	...
      	site_t site_Count_OutletWall_Inner = mLatDat->GetMidDomainCollisionCount(5);
				...
				*/
				...
				// 2. Copy all lattice constants to GPU constant memory.
				// AMD HIP fix: hipMemcpyToSymbol fails cross-TU for extern __constant__ symbols,
				// so all copies are done inside cuda_params.cu (same TU as the definitions).
				...
				double minus_inv_tau = mParams.GetOmega();
				distribn_t iStressParameter = mParams.GetStressParameter();

				status = hemelb::initGPUSymbolConstants(
					LatticeType::EQMWEIGHTS, num_Vectors,
					LatticeType::INVERSEDIRECTIONS,
					LatticeType::CX, LatticeType::CY, LatticeType::CZ,
					tau, minus_inv_tau, Cs2,
					useWeightsFromFile, (double)iStressParameter);
				if (!status) {
					fprintf(stderr, "GPU constant memory copy failed\n");
					initialise_GPU_res = false;
					return initialise_GPU_res;
				}
				...
				if(nBlocks_Collide!=0){
					if (hemeKernel == "LBGKSL"){
						hemelb::GPU_CollideStream_mMidFluidCollision_mWallCollision_sBB_WallShearStress<<<nBlocks_Collide, nThreads_Collide, 0, Collide_Stream_PreSend_1>>> (
								...
								(distribn_t*)mLatDat->GPUDataAddr_dbl_fNew_b_mLatDat,
								(distribn_t*)GPUDataAddr_dbl_MacroVars,
								(int32_t*)GPUDataAddr_int64_Neigh_d,
